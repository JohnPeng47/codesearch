from codesearch.moatless.loop.base import Loop, Rejected, Finished
from codesearch.moatless.loop.clarify import ClarifyCodeChange
from codesearch.moatless.loop.writecode import PlanToCode
from codesearch.moatless.loop.old import RequestForChangeRequest
from codesearch.moatless.loop.writecode import WriteCode
from codesearch.moatless.types import RejectRequest, FinishRequest
from .utils import create_workspace


def create_coder(
    mocker, instance_id: str, file_path: str, span_ids: set[str], instructions: str = ""
):
    coding = PlanToCode(instructions=instructions)
...
def test_request_for_change(mocker):
    ...
    coding, mock_loop = create_coder(
        mocker, instance_id=instance_id, file_path=file_path, span_ids={span_id}
    )

    request = RequestForChangeRequest(
        file_path=file_path, span_id=span_id, description="I want to update this"
    )

    response = coding.handle_action(request)
    assert response is None
    mock_loop.transition_to.assert_called_once_with(
        WriteCode(
...
def test_clarify_changes(mocker):
    ...
    mock_loop.transition_to.assert_called_once_with(
        ClarifyCodeChange(
            description=request.description, file_path=file_path, span_id=span_id
        )
    )
